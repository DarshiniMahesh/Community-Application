...
const SECTION_MAP: Record<string, { tab: "users" | "sanghas"; domId: string }> = {
  demographics:    { tab: "users",   domId: "au-demographics"  },
  geographic:      { tab: "users",   domId: "au-geographic"    },
  education:       { tab: "users",   domId: "au-education"     },
  economic:        { tab: "users",   domId: "au-economic"      },
  assets:          { tab: "users",   domId: "au-assets"        },
  insurance:       { tab: "users",   domId: "au-insurance"     },
  documents:       { tab: "users",   domId: "au-documents"     },
  religious:       { tab: "users",   domId: "au-religious"     },
  status:          { tab: "users",   domId: "au-status"        },
  sanghas:         { tab: "sanghas", domId: "as-overview"      },
  "as-overview":   { tab: "sanghas", domId: "as-overview"      },
  "as-status":     { tab: "sanghas", domId: "as-status"        },
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg:#f4f6f9;--surface:#ffffff;--border:#e2e8f0;--border2:#edf2f7;
    --text-1:#111827;--text-2:#374151;--text-3:#374151;--text-4:#6b7280;
    --accent:#4f46e5;--accent-lt:#eef2ff;--accent-md:#c7d2fe;
    --sky:#0284c7;--sky-lt:#e0f2fe;--teal:#0d9488;--teal-lt:#ccfbf1;
    --amber:#d97706;--amber-lt:#fef3c7;--rose:#e11d48;--rose-lt:#ffe4e6;
    --emerald:#059669;--emerald-lt:#d1fae5;--violet:#7c3aed;--violet-lt:#ede9fe;
    --orange:#ea580c;--orange-lt:#fff7ed;
    --radius-sm:8px;--radius:12px;--radius-lg:18px;
    --shadow-sm:0 1px 3px rgba(0,0,0,.06),0 1px 2px rgba(0,0,0,.04);
    --shadow:0 4px 12px rgba(0,0,0,.08);--shadow-lg:0 8px 24px rgba(0,0,0,.1);
    --font:'DM Sans',sans-serif;--mono:'DM Mono',monospace;--transition:180ms ease;
  }
  body{font-family:var(--font);background:var(--bg);color:var(--text-1);}
  .adv-root{min-height:100vh;background:var(--bg);}
  .adv-topbar{background:var(--surface);border-bottom:1px solid var(--border);padding:14px 28px;position:sticky;top:0;z-index:30;display:flex;align-items:center;justify-content:space-between;gap:16px;}
  ...
  .adv-tab-group{display:flex;align-items:center;gap:4px;background:var(--bg);border-radius:var(--radius);padding:4px;border:1px solid var(--border);}
  .adv-tab-btn{display:flex;align-items:center;gap:7px;padding:8px 18px;border-radius:var(--radius-sm);font-size:13px;font-weight:600;cursor:pointer;border:none;background:transparent;color:var(--text-3);transition:all var(--transition);}
  .adv-tab-btn.active{background:var(--surface);color:var(--text-1);box-shadow:var(--shadow-sm);}
  .adv-tab-btn:hover:not(.active){color:var(--text-2);}
  .adv-refresh-btn{display:flex;align-items:center;gap:6px;padding:8px 14px;border-radius:var(--radius-sm);font-size:12px;font-weight:600;cursor:pointer;border:1px solid var(--border);background:var(--surface);color:var(--text-3);transition:all var(--transition);}
  .adv-refresh-btn:hover{border-color:var(--accent-md);color:var(--accent);background:var(--accent-lt);}
  .adv-error-bar{display:flex;align-items:center;gap:10px;border:1px solid #fecaca;background:#fff5f5;border-radius:var(--radius);padding:12px 18px;font-size:13px;color:#b91c1c;margin:16px 28px 0;}
  .adv-error-retry{margin-left:auto;font-size:11px;font-weight:600;color:#dc2626;text-decoration:underline;cursor:pointer;border:none;background:none;}
  .adv-body{max-width:1600px;margin:0 auto;padding:28px;}
  .adv-flex{display:flex;gap:0;}
  .adv-sidebar{display:none;width:210px;flex-shrink:0;flex-direction:column;gap:2px;position:sticky;top:76px;align-self:flex-start;padding-right:20px;}
  @media(min-width:1280px){.adv-sidebar{display:flex;}}
  .adv-sidebar-label{font-size:10px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.12em;margin-bottom:8px;padding-left:8px;}
  .adv-nav-btn{display:flex;align-items:center;gap:8px;padding:9px 10px;border-radius:var(--radius-sm);font-size:12px;font-weight:500;cursor:pointer;border:none;background:transparent;color:#374151;text-align:left;transition:all var(--transition);width:100%;}
  .adv-nav-btn:hover:not(.active){background:var(--bg);color:#111827;}
  .adv-nav-btn.active{background:var(--bg);color:#111827;font-weight:700;}
  ...
  .adv-section{scroll-margin-top:90px;transition:outline 0.3s ease;}
  @keyframes section-flash{0%{outline:2px solid #0ea5e9;outline-offset:6px;border-radius:16px;}60%{outline:2px solid #0ea5e9;outline-offset:6px;border-radius:16px;}100%{outline:2px solid transparent;outline-offset:6px;border-radius:16px;}}
  .adv-section.highlight{animation:section-flash 1.6s ease forwards;}
  .adv-banner{display:flex;align-items:center;gap:10px;border-radius:var(--radius);padding:12px 18px;font-size:13px;color:var(--text-2);}
  ...
  .adv-sec-title{font-size:16px;font-weight:800;color:#111827;letter-spacing:-.3px;}
  .adv-sec-sub{font-size:12px;color:#374151;margin-top:2px;}
  .adv-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;box-shadow:var(--shadow-sm);}
  .adv-card-head{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;}
  .adv-card-title{font-size:14px;font-weight:700;color:#111827;}
  .adv-card-sub{font-size:11px;color:#374151;margin-top:2px;}
  .adv-card-badge{padding:3px 9px;border-radius:20px;font-size:10px;font-weight:700;background:var(--accent-lt);color:var(--accent);border:1px solid var(--accent-md);white-space:nowrap;flex-shrink:0;}
  .adv-card-actions{display:flex;gap:8px;margin-top:14px;padding-top:12px;border-top:1px solid var(--border2);}
  .adv-card-action-btn{flex:1;display:flex;align-items:center;justify-content:center;gap:5px;padding:6px 0;border-radius:8px;font-size:11px;font-weight:600;border:1px solid var(--border);background:var(--surface);color:#374151;cursor:pointer;transition:all 0.15s;}
  .adv-card-action-btn:hover{border-color:#bae6fd;background:#f0f9ff;color:#0284c7;}
  .adv-card-action-btn.report:hover{border-color:#ddd6fe;background:#f5f3ff;color:#7c3aed;}
  .kpi-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px;box-shadow:var(--shadow-sm);}
  .kpi-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;}
  .kpi-label{font-size:12px;font-weight:600;color:#374151;}
  ...
  .hero-banner{border-radius:var(--radius-lg);padding:24px;color:#fff;position:relative;overflow:hidden;box-shadow:var(--shadow-lg);margin-bottom:20px;}
  .hero-banner::before{content:'';position:absolute;width:140px;height:140px;border-radius:50%;background:rgba(255,255,255,.08);top:-28px;right:-28px;}
  ...
  .asset-tile{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px;text-align:center;box-shadow:var(--shadow-sm);}
  .asset-tile-label{font-size:12px;color:#374151;font-weight:600;margin-bottom:12px;}
  ...
  .asset-ring-pct{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;color:var(--text-1);font-family:var(--mono);}
  .asset-tile-count{font-size:14px;font-weight:800;color:var(--text-1);margin-top:8px;font-family:var(--mono);}
  .asset-tile-total{font-size:11px;color:#374151;margin-top:2px;}
  .doc-holder-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;}
  @media(max-width:900px){.doc-holder-grid{grid-template-columns:repeat(2,1fr);}}
  @media(max-width:1100px){.doc-holder-grid{grid-template-columns:repeat(3,1fr);}}
  .doc-holder-tile{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px 16px;box-shadow:var(--shadow-sm);display:flex;flex-direction:column;gap:12px;}
  .doc-holder-label{font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.06em;}
  .doc-holder-pct{font-size:30px;font-weight:900;color:var(--text-1);font-family:var(--mono);letter-spacing:-1px;line-height:1;}
  .doc-holder-bar{height:6px;background:#f1f5f9;border-radius:99px;overflow:hidden;}
  .doc-holder-bar-fill{height:6px;border-radius:99px;transition:width .6s ease;}
  .doc-holder-rows{display:flex;flex-direction:column;gap:4px;margin-top:2px;}
  .doc-holder-row{display:flex;align-items:center;justify-content:space-between;font-size:10px;font-weight:600;}
  .doc-holder-dot{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
  ...
  .gender-pill-name{font-size:11px;color:#374151;font-weight:600;}
  .gender-pill-value{font-size:18px;font-weight:900;font-family:var(--mono);margin-top:2px;}
  .gender-pill-pct{font-size:10px;color:#374151;margin-top:1px;}
  ...
  .ftype-count{font-size:11px;color:#374151;margin-top:2px;}
  .marital-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-top:10px;}
  .marital-pill{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:var(--radius-sm);background:var(--bg);}
  .marital-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
  .marital-label{font-size:11px;color:#374151;font-weight:600;flex:1;}
  .marital-count{font-size:14px;font-weight:900;font-family:var(--mono);color:var(--text-1);}
  .status-pills{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px;}
  .status-pill{display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-radius:var(--radius-sm);background:var(--bg);}
  .status-pill-left{display:flex;align-items:center;gap:7px;}
  .status-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0;}
  .status-name{font-size:12px;color:#374151;font-weight:500;}
  .status-count{font-size:12px;font-weight:800;color:#111827;font-family:var(--mono);}
  .geo-filter-bar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;}
  .geo-filter-btn{display:flex;align-items:center;gap:6px;padding:8px 14px;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--surface);font-size:12px;font-weight:600;color:#374151;cursor:pointer;transition:all var(--transition);}
  ...
  .geo-dropdown{position:absolute;left:0;top:calc(100% + 6px);z-index:50;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:var(--shadow-lg);width:240px;max-height:260px;overflow-y:auto;padding:10px;}
  .geo-dropdown-item{display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:var(--radius-sm);font-size:12px;color:#374151;cursor:pointer;}
  .geo-dropdown-item:hover{background:var(--bg);}
  .geo-dropdown-item input{accent-color:var(--teal);}
  .geo-dropdown-item-count{margin-left:auto;font-size:10px;color:#374151;background:var(--bg);border-radius:20px;padding:1px 7px;}
  ...
  .skeleton{border-radius:var(--radius-lg);background:linear-gradient(90deg,#e2e8f0 25%,#edf2f7 50%,#e2e8f0 75%);background-size:200% 100%;animation:shimmer 1.4s infinite;}
  ...
  .top-table th{padding:8px 8px 10px;text-align:left;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.06em;}
  .top-table th.right{text-align:right;}.top-table th.center{text-align:center;}
  ...
  .top-table td.right{text-align:right;}.top-table td.center{text-align:center;}
  .top-table td.rank{color:#374151;font-weight:700;font-family:var(--mono);font-size:11px;}
  .top-table td.name{font-weight:700;color:#111827;}
  .top-table td.loc{color:#374151;}
  .top-table td.total{font-weight:800;color:#111827;font-family:var(--mono);}
  ...
  .no-data{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;padding:48px 24px;color:#374151;font-size:13px;border:1.5px dashed var(--border);border-radius:var(--radius-lg);background:var(--surface);}
...
export interface AdminSanghaReport {
  ...
  sanghasMemberTypeBreakdown?: Array<{
    sangha_name:     string;
    state:           string;
    fulltime_male:   number;
    fulltime_female: number;
    fulltime_other:  number;
    parttime_male:   number;
    parttime_female: number;
...
const EXCLUDED_STATUS_KEYS = new Set(["draft", "submitted"]);
...
const DOC_COLORS    = ["#0d9488","#0891b2","#6366f1","#8b5cf6","#f59e0b"];
...
const AXIS_X_COLOR   = "#111827";
const AXIS_Y_COLOR   = "#111827";
...
const USER_NAV = [
  ...
  { id: "au-documents",    label: "Document Holders",       icon: FileText,      color: "#f43f5e" },
...
function CardActions({ onReport, reportLabel = "Custom Report" }: { onReport?: () => void; reportLabel?: string }) {
  if (!onReport) return null;
  return (
    <div className="adv-card-actions">
      <button className="adv-card-action-btn report" onClick={onReport}>
...
function Card({ title, subtitle, children, className = "", badge, style, onReport, reportLabel }: {
  title: string; subtitle?: string; children: React.ReactNode;
  className?: string; badge?: string; style?: React.CSSProperties;
  onReport?: () => void; reportLabel?: string;
}) {
  return (
    <div className={`adv-card ${className}`} style={style}>
      ...
      {children}
      <CardActions onReport={onReport} reportLabel={reportLabel} />
    </div>
  );
}
...
function GenderBar({ data, height = 220 }: { data: { label: string; male: number; female: number; other?: number }[]; height?: number }) {
  if (!data?.length) return <div style={{ height, display: "flex", alignItems: "center", justifyContent: "center", color: "#374151", fontSize: 13 }}>No data</div>;
  const hasOther = data.some(d => (d.other ?? 0) > 0);
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ left: -20, right: 5, top: 5, bottom: 5 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
        <XAxis dataKey="label" tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_X_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} />
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ left: 8, right: 20, top: 5, bottom: 5 }}>
        ...
        <YAxis dataKey="label" type="category" width={195} tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} />
        <Tooltip content={<BarTip />} />
        <Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: "12px", color: "#111827", fontWeight: 600 }} />
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart layout="vertical" data={data} margin={{ left: 8, right: 30, top: 5, bottom: 5 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
        <XAxis type="number" tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_X_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} allowDecimals={false} />
        <YAxis dataKey="label" type="category" width={140} tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} />
        <Tooltip content={<BarTip />} />
        <Bar dataKey="count" name="Count" radius={[0,6,6,0]}>
          {data.map((_: any, i: number) => <Cell key={i} fill={palette[i % palette.length]} />)}
          <LabelList dataKey="count" position="right" style={{ fontSize: 11, fontWeight: 700, fill: "#111827" }} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
function useScrollToSection(
  ...
  activeTab: "users" | "sanghas",
  setActiveTab: (t: "users" | "sanghas") => void,
  onSectionRendered?: () => void,
) {
  const pendingRef = useRef<string | null>(null);
  useEffect(() => {
    if (!initialSection) return;
    const mapping = SECTION_MAP[initialSection];
    if (!mapping) return;
    pendingRef.current = mapping.domId;
    if (activeTab !== mapping.tab) setActiveTab(mapping.tab);
    else scrollAndHighlight(mapping.domId, onSectionRendered);
  }, [initialSection]);
  useEffect(() => {
    if (!pendingRef.current) return;
    const domId = pendingRef.current;
    pendingRef.current = null;
    scrollAndHighlight(domId, onSectionRendered, 350);
  }, [activeTab]);
}

function scrollAndHighlight(domId: string, cb?: () => void, extraDelay = 0) {
  const attempt = (attemptsLeft: number) => {
    ...
    if (el) {
      ...
      setTimeout(() => attempt(attemptsLeft - 1), 120);
    }
  };
  setTimeout(() => attempt(8), extraDelay);
}

// ─── SanghaMembersChart ───────────────────────────────────────────────────────
function SanghaMembersChart({
  data,
  onReport,
}: {
  data: AdminSanghaReport;
  onReport?: () => void;
}) {
  const ROLES = [
    { key: "president",       label: "President",      color: "#6366f1" },
    { key: "vice_president",  label: "Hon. President", color: "#a855f7" },
    { key: "secretary",       label: "Secretary",      color: "#0ea5e9" },
    { key: "joint_secretary", label: "Hon. Secretary", color: "#14b8a6" },
    { key: "treasurer",       label: "Treasurer",      color: "#10b981" },
  ...
  const sorted = useMemo(
    () => [...(data.sanghasByMemberCount || [])].sort((a, b) => b.total_members - a.total_members),
    [data.sanghasByMemberCount]
  );

  const [selected, setSelected] = useState<string[]>(() =>
    sorted.slice(0, 5).map(s => s.sangha_name)
  );
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (addRef.current && !addRef.current.contains(e.target as Node)) setDdOpen(false);
    };
  ...
  const maxTotal = useMemo(
    () => Math.max(...selected.map(n => sorted.find(s => s.sangha_name === n)?.total_members ?? 0), 1),
    [selected, sorted]
  );

  const usedRoles = ROLES.filter(r =>
    selected.some(n => (sorted.find(s => s.sangha_name === n) as any)?.[r.key] > 0)
  );

  if (!sorted.length) {
    return <div className="no-data"><span>No sangha member data</span></div>;
  }

  return (
    <Card
      title="Members per Sangha by Role"
      subtitle="Each bar = one sangha · segments = roles · number = count in that role"
      badge={`${sorted.length} sangha${sorted.length !== 1 ? "s" : ""}`}
      onReport={onReport}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#374151", flexShrink: 0 }}>Sanghas:</span>
        {selected.map(name => (
          <span key={name} style={{
            display: "inline-flex", alignItems: "center", gap: 4,
            padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 500,
            background: "#e0f2fe", border: "0.5px solid #0ea5e9", color: "#0369a1",
          }}>
            {name.length > 18 ? name.slice(0, 17) + "…" : name}
            {selected.length > 1 && (
              <button
                onClick={() => setSelected(p => p.filter(n => n !== name))}
                style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, lineHeight: 1, color: "#0369a1", padding: 0 }}
              >×</button>
            )}
          </span>
        ))}
        <div ref={addRef} style={{ position: "relative" }}>
          <button
            ...
            style={{
              ...
              border: "0.5px dashed #cbd5e1", background: "transparent", color: "#374151",
            }}
          >+ Add sangha</button>
          {ddOpen && (
            <div style={{
              ...
              minWidth: 200, maxHeight: 220, overflowY: "auto", padding: 6,
              boxShadow: "0 4px 12px rgba(0,0,0,.08)",
            }}>
              {sorted.map(row => {
                const isSel = selected.includes(row.sangha_name);
                return (
                  <div
                    key={row.sangha_name}
                    onClick={() => {
                      if (isSel) {
                        if (selected.length > 1) setSelected(p => p.filter(n => n !== row.sangha_name));
                      } else {
                        setSelected(p => [...p, row.sangha_name]);
                      }
                    }}
                    style={{
                      display: "flex", alignItems: "center", justifyContent: "space-between",
                      padding: "7px 10px", borderRadius: 8, fontSize: 12, cursor: "pointer",
                      color: isSel ? "#0284c7" : "#374151", background: "transparent",
                    }}
                    onMouseEnter={e => (e.currentTarget.style.background = "#f8fafc")}
                    onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                  >
                    <span>{row.sangha_name}</span>
                    <span style={{ fontSize: 10, fontWeight: 500, color: "#374151", background: "#f1f5f9", padding: "1px 6px", borderRadius: 10 }}>
                      {row.total_members}
                    </span>
                  </div>
                );
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {selected.map(name => {
          const row = sorted.find(s => s.sangha_name === name);
          if (!row) return null;
          const activeRoles = ROLES.filter(r => (row as any)[r.key as RoleKey] > 0);
          return (
            <div key={name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div title={row.sangha_name} style={{
                width: 160, flexShrink: 0, textAlign: "right",
                fontSize: 12, fontWeight: 600, color: "#374151",
                overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
              }}>
                {row.sangha_name.length > 22 ? row.sangha_name.slice(0, 21) + "…" : row.sangha_name}
              </div>
              <div style={{
                flex: 1, height: 32, display: "flex", borderRadius: 6,
                overflow: "hidden", background: "#f1f5f9", minWidth: 0,
              }}>
                {activeRoles.length === 0 ? (
                  <div style={{ fontSize: 11, color: "#374151", padding: "0 8px", display: "flex", alignItems: "center" }}>No role data</div>
                ) : (
                  activeRoles.map((role, i) => {
                    const v = (row as any)[role.key as RoleKey] as number;
                    const pct = Math.round((v / maxTotal) * 100);
                    const isFirst = i === 0;
                    const isLast = i === activeRoles.length - 1;
                    const radius = isFirst && isLast ? 6 : isFirst ? "6px 0 0 6px" : isLast ? "0 6px 6px 0" : 0;
                    return (
                      <div key={role.key} title={`${role.label}: ${v}`} style={{
                        width: `${pct}%`, minWidth: v > 0 ? 18 : 0, background: role.color,
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 10, fontWeight: 500, color: "rgba(255,255,255,0.9)",
                        overflow: "hidden", whiteSpace: "nowrap", borderRadius: radius,
              ...
              <div style={{ width: 28, flexShrink: 0, fontSize: 12, fontWeight: 600, color: "#374151" }}>
                {row.total_members}
              </div>
            </div>
          );
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 16px", marginTop: 18, paddingTop: 14, borderTop: "0.5px solid #e2e8f0" }}>
        {usedRoles.map(role => (
          <span key={role.key} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "#374151", fontWeight: 500 }}>
            <span style={{ width: 10, height: 10, borderRadius: 2, background: role.color, flexShrink: 0, display: "inline-block" }} />
            {role.label}
          </span>
        ))}
      </div>
...
function SanghaMemberTypeChart({
  ...
  const rows = useMemo(
    () => [...(data.sanghasMemberTypeBreakdown || [])].sort((a, b) => b.total - a.total),
    [data.sanghasMemberTypeBreakdown]
  );
...
function TopSanghasChart({
  ...
  const sorted = useMemo(
    () => [...(data.topSanghas || [])].sort((a, b) => b.total_users - a.total_users).slice(0, 20),
    [data.topSanghas]
  );
...
// ─── UserTab ──────────────────────────────────────────────────────────────────
function UserTab({ data, loading, onGoToCustomReport }: {
  data: AdminUserReport | null;
  loading: boolean;
  onGoToCustomReport?: (sections: string[], category?: string) => void;
}) {
  ...
  const statusGenderBarData = (data.statusGenderBreakdown || [])
    .filter(s => !EXCLUDED_STATUS_KEYS.has(s.status.toLowerCase()))
    .map(s => ({ label: STATUS_LABELS[s.status] ?? s.status, male: s.male || 0, female: s.female || 0, other: s.other || 0 }));
  ...
  const maritalBarData = maritalGenderData.length > 0
    ? maritalGenderData
    : dem.maritalStatus.map((m: any) => ({ label: m.label, male: m.count, female: 0, other: 0 }));
  ...
  const extraDegrees = rawDegreesGender.filter(d => !DEGREE_ORDER.includes(d.label));
  const degreesBarData = [
    ...DEGREE_ORDER.map(label => degreeMap.get(label) ?? { label, male: 0, female: 0, other: 0 }),
    ...extraDegrees,
  ].filter(d => d.male + d.female + d.other > 0 || DEGREE_ORDER.includes(d.label));

  const professionGenderData = (edu.professionsGender || []).map((p: any) => ({ label: p.label, male: p.male || 0, female: p.female || 0, other: p.other || 0 }));
  const professionBarData = professionGenderData.length > 0
    ? professionGenderData
    : (edu.professions || []).map((p: any) => ({ label: p.label, male: p.count, female: 0, other: 0 }));
  ...
  const employmentBarData = employmentGenderData.length > 0
    ? employmentGenderData
    : (eco.employment || []).map((e: any) => ({ label: e.label, male: e.count, female: 0, other: 0 }));
    ...
    <div className="adv-flex">
      ...
      <div className="adv-content">
        <div className="adv-banner indigo">
          ...
          <span>Platform-wide user analytics — <strong style={{ color: "#4f46e5" }}>all approved profiles including family members across every sangha.</strong></span>
        </div>
        ...
        <section className="adv-section" id="au-status">
          ...
          {statusGenderBarData.length > 0 ? (
            <Card title="Status by Gender" subtitle="Approved · Under Review · Changes Requested · Rejected — Male (blue) · Female (pink)"
              onReport={() => goCustom?.(["personal-details"], "user")}>
              <GenderBar data={statusGenderBarData} height={200} />
            </Card>
          ) : (
            <Card title="Status Overview" onReport={() => goCustom?.(["personal-details"], "user")}>
              <div style={{ fontSize: 42, fontWeight: 900, color: "#111827", fontFamily: "var(--mono)", textAlign: "center" }}>{data.totalApproved.toLocaleString()}</div>
              <div style={{ fontSize: 12, color: "#374151", marginTop: 4, textAlign: "center" }}>Approved profiles</div>
            </Card>
          )}
        </section>
        ...
        <section className="adv-section" id="au-demographics">
          <div className="adv-sec-header">
            ...
            <div><div className="adv-sec-title">Population & Demographics</div><div className="adv-sec-sub">Gender, marital status, age groups & family type — approved users + family members</div></div>
          </div>

          <div className="hero-banner hero-sky">
            <div className="hero-eyebrow">Total Platform Population (Users + Family Members)</div>
          ...
          <div className="grid-3">
            ...
            <Card title="Gender Distribution" subtitle="Approved users — Male · Female · Other"
              onReport={() => goCustom?.(["personal-details"], "user")}>
              {genderData.length === 0 ? (
                <div style={{ height: 160, display: "flex", alignItems: "center", justifyContent: "center", color: "#374151", fontSize: 13 }}>No gender data</div>
              ) : (
                <>
                  <ResponsiveContainer width="100%" height={150}>
                    <PieChart>
                      <Pie data={genderData} cx="50%" cy="50%" innerRadius={45} outerRadius={65} paddingAngle={3} dataKey="value" strokeWidth={2} stroke="#fff">
            ...
            <Card title="Family Type" subtitle="Nuclear vs Joint — approved families"
              ...
              <ResponsiveContainer width="100%" height={120}>
                <PieChart>
                  <Pie data={ftData} cx="50%" cy="50%" innerRadius={38} outerRadius={55} paddingAngle={4} dataKey="value" strokeWidth={2} stroke="#fff">
              ...
              <div className="ftype-pills">
                {ftData.map(d => (
                  <div key={d.name} className="ftype-pill">
                    ...
                    <div className="ftype-count">{d.value.toLocaleString()} families</div>
                  </div>
                ))}
              </div>
            </Card>
      ...
      >
        ...
        <section className="adv-section" id="au-education">
          <div className="adv-sec-header">
            ...
            <div><div className="adv-sec-title">Education & Occupation</div><div className="adv-sec-sub">All 7 degree levels, professions, study/work status — approved Male · Female · Other users</div></div>
          </div>
          <div className="grid-2">
            <Card title="Highest Degree Level" subtitle="Approved users — Male · Female · Other across all 7 levels"
              onReport={() => goCustom?.(["education-profession"], "user")}>
              <GenderBarV data={degreesBarData} height={Math.max(280, DEGREE_ORDER.length * 38)} />
            </Card>
            <Card title="Profession Breakdown" subtitle="Approved users — Male · Female · Other"
              onReport={() => goCustom?.(["education-profession"], "user")}>
              <GenderBarV data={professionBarData} height={Math.max(220, professionBarData.length * 32)} />
            </Card>
          </div>
          <div className="grid-2 mt-5">
            <Card title="Currently Studying" subtitle="Approved users — Male · Female"
              onReport={() => goCustom?.(["education-profession"], "user")}>
              <GenderBar data={studyingBarData} height={150} />
            </Card>
            <Card title="Currently Working" subtitle="Approved users — Male · Female"
              onReport={() => goCustom?.(["education-profession"], "user")}>
        ...
        <section className="adv-section" id="au-geographic">
          <div className="adv-sec-header">
            ...
            <div><div className="adv-sec-title">Geographic Distribution</div><div className="adv-sec-sub">Cities — users and their family members</div></div>
          </div>
          ...
          <Card
            title={selectedCities.length === 0 ? "Top Cities by Population (Users + Family Members)" : `${selectedCities.length} Cities Selected`}
            subtitle="Male · Female — geographic spread including family members"
            onReport={() => goCustom?.(["location-information"], "user")}
          >
            {geoGenderData.length > 0 ? (
              <GenderBar data={geoGenderData} height={260} />
            ) : filteredGeo.length === 0 ? (
              <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", color: "#374151", fontSize: 13 }}>No data</div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={filteredGeo} margin={{ top: 20, right: 24, left: -16, bottom: 4 }} barCategoryGap="30%">
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="city" tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_X_COLOR, fontWeight: 600 }} axisLine={false} tickLine={false}
                    tickFormatter={(v) => typeof v === "string" ? (v.length > 12 ? v.slice(0, 11) + "…" : v) : v} />
                  <YAxis allowDecimals={false} tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} axisLine={false} tickLine={false} width={28} />
                  <Tooltip content={<BarTip />} />
                  <Bar dataKey="count" radius={[8,8,0,0]} maxBarSize={72} name="Population">
        ...
        <section className="adv-section" id="au-economic">
          ...
          {eco.incomeSlabs.length === 0 ? (
            ...
            <Card title="Family Income Distribution" subtitle="Annual household income brackets"
              onReport={() => goCustom?.(["economic-details"], "user")}>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={eco.incomeSlabs} margin={{ left: -20, right: 5, top: 5, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  <XAxis dataKey="label" tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_X_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} />
                  <YAxis tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} />
                  <Tooltip content={<BarTip />} />
        ...
        <section className="adv-section" id="au-assets">
            ...
            {assetsGenderData.length > 0 ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={eco.assets.map(a => ({ name: a.label, "Owned %": a.total > 0 ? Math.round(a.owned / a.total * 100) : 0 }))} margin={{ left: -20, right: 5, top: 5, bottom: 5 }}>
                  ...
                  <YAxis tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} unit="%" domain={[0,100]} />
                  <Tooltip content={<BarTip />} />
          ...
          {employmentBarData.length > 0 && (
            <Card title="Employment Sector Breakdown" subtitle="Approved users — Male · Female — Govt, Private, Self-Employed"
              style={{ marginTop: 18 }} onReport={() => goCustom?.(["education-profession"], "user")}>
        ...
        <section className="adv-section" id="au-insurance">
          <div className="adv-sec-header">
            ...
            <div>
              <div className="adv-sec-title">Insurance Coverage</div>
              <div className="adv-sec-sub">Health · Life · Term · Konkani Card — Yes · No · Not Selected</div>
            </div>
          </div>
          {data.insurance.length > 0 ? (
            <div className="insurance-grid">
              {data.insurance.map(ins => {
                const totalYes = (ins.maleYes || 0) + (ins.femaleYes || 0) + (ins.otherYes || 0);
                ...
                return (
                  <Card key={ins.label} title={ins.label}
                    subtitle={`${totalYes.toLocaleString()} covered · ${totalNo.toLocaleString()} not covered · ${totalUnk.toLocaleString()} not selected`}
                    onReport={() => goCustom?.(["insurance"], "user")}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, padding: "8px 12px", background: "var(--bg)", borderRadius: "var(--radius-sm)" }}>
                      <div style={{ flex: 1, height: 8, background: "#f1f5f9", borderRadius: 99, overflow: "hidden", display: "flex" }}>
                        {totalAll > 0 && (
                          <>
                            <div style={{ height: 8, background: "linear-gradient(90deg,#0d9488,#14b8a6)", width: `${Math.round(totalYes / totalAll * 100)}%`, transition: "width .6s ease" }} />
                            <div style={{ height: 8, background: "#f43f5e", width: `${Math.round(totalNo / totalAll * 100)}%`, transition: "width .6s ease" }} />
                            <div style={{ height: 8, background: "#cbd5e1", width: `${Math.round(totalUnk / totalAll * 100)}%`, transition: "width .6s ease" }} />
                          </>
                        )}
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#0d9488", fontFamily: "var(--mono)", flexShrink: 0 }}>{coveragePct}%</span>
                    </div>
                    <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                      {[
                        { label: "Yes", val: totalYes, bg: "#ccfbf1", color: "#0d9488" },
                        { label: "No",  val: totalNo,  bg: "#ffe4e6", color: "#e11d48" },
                        { label: "N/A", val: totalUnk, bg: "#f1f5f9", color: "#374151" },
                      ].map(item => (
                        <div key={item.label} style={{ flex: 1, textAlign: "center", padding: "8px 4px", borderRadius: "var(--radius-sm)", background: item.bg }}>
                          <div style={{ fontSize: 18, fontWeight: 900, color: item.color, fontFamily: "var(--mono)" }}>{item.val.toLocaleString()}</div>
                          <div style={{ fontSize: 9, color: item.color, fontWeight: 700, marginTop: 2, opacity: 0.8 }}>{item.label}</div>
                        </div>
                      ))}
                    </div>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={chartData} margin={{ top: 4, right: 12, left: -10, bottom: 4 }} barCategoryGap="30%" barGap={3}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                        <XAxis dataKey="status" tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_X_COLOR, fontWeight: 600 }} axisLine={false} tickLine={false} />
                        <YAxis allowDecimals={false} tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<BarTip />} />
                        <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 12, paddingTop: 8, color: "#111827", fontWeight: 600 }} />
        ...
        <section className="adv-section" id="au-documents">
          <div className="adv-sec-header">
            ...
            <div><div className="adv-sec-title">Document Holders</div><div className="adv-sec-sub">Aadhaar, PAN, Voter ID, Land Records, DL — approved users</div></div>
          </div>
          {data.documents.length > 0 ? (
            <>
              <div className="doc-holder-grid">
                {data.documents.map((doc, idx) => {
                  const t = (doc.yes + doc.no + doc.unknown) || 1;
                  const pct = Math.round(doc.yes / t * 100);
                  const color = DOC_COLORS[idx % DOC_COLORS.length];
                  return (
                    <div key={doc.label} className="doc-holder-tile" style={{ borderTop: `3px solid ${color}` }}>
                      <div className="doc-holder-label" style={{ color }}>{doc.label}</div>
                      <div>
                        <div className="doc-holder-pct" style={{ color }}>{pct}%</div>
                        <div style={{ fontSize: 11, color: "#374151", marginTop: 2 }}>{doc.yes.toLocaleString()} holders</div>
                      </div>
                      <div className="doc-holder-bar">
                        <div className="doc-holder-bar-fill" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${color}, ${color}99)` }} />
                      </div>
                      <div className="doc-holder-rows">
                        <div className="doc-holder-row">
                          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <div className="doc-holder-dot" style={{ background: "#22c55e" }} />
                            <span style={{ color: "#22c55e" }}>Holders</span>
                          </div>
                          <span style={{ color: "#22c55e" }}>{doc.yes.toLocaleString()}</span>
                        </div>
                        <div className="doc-holder-row">
                          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <div className="doc-holder-dot" style={{ background: "#ef4444" }} />
                            <span style={{ color: "#ef4444" }}>Non-Holders</span>
                          </div>
                          <span style={{ color: "#ef4444" }}>{doc.no.toLocaleString()}</span>
                        </div>
                        <div className="doc-holder-row">
                          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <div className="doc-holder-dot" style={{ background: "#94a3b8" }} />
                            <span style={{ color: "#374151" }}>Unknown</span>
                          </div>
                          <span style={{ color: "#374151" }}>{doc.unknown.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <Card title="Document Holders Comparison" subtitle="Holders · Non-Holders · Unknown — approved users"
                ...
                <ResponsiveContainer width="100%" height={240}>
                  ...
                  >
                    ...
                    <YAxis tick={{ fontSize: AXIS_FONT_SIZE, fill: AXIS_Y_COLOR, fontWeight: 600 }} tickLine={false} axisLine={false} allowDecimals={false} />
                    <Tooltip content={<BarTip />} />
                    <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: "12px", color: "#111827", fontWeight: 600 }} />
                    <Bar dataKey="Holders"     fill="#22c55e" radius={[4,4,0,0]} maxBarSize={36} />
                    <Bar dataKey="Non-Holders" fill="#ef4444" radius={[4,4,0,0]} maxBarSize={36} />
                    <Bar dataKey="Unknown"     fill="#94a3b8" radius={[4,4,0,0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </>
        ...
        <section className="adv-section" id="au-religious">
          ...
          {!gotraData.length && !kuldevData.length && !upanamaGenData.length && !demiGodData.length ? (
            ...
            <>
              <div className="grid-2">
                {gotraData.length > 0 && (
                  <Card title="Gotras" subtitle="Most common gotras" onReport={() => goCustom?.(["religious-details"], "user")}>
                    <SimpleBar data={gotraData} palette={RELIG_PALETTE} height={Math.max(220, gotraData.length * 30)} />
                  </Card>
                )}
                {kuldevData.length > 0 && (
                  <Card title="Kuladevata Distribution" subtitle="Family deity distribution" onReport={() => goCustom?.(["religious-details"], "user")}>
                    <SimpleBar data={kuldevData} palette={RELIG_PALETTE.slice(3)} height={Math.max(220, kuldevData.length * 30)} />
                  </Card>
                )}
              </div>
              {pravaraData.length > 0 && (
                <Card title="Pravara Distribution" subtitle="Pravara lineages" style={{ marginTop: 18 }} onReport={() => goCustom?.(["religious-details"], "user")}>
              ...
              {(upanamaGenData.length > 0 || upanaPropData.length > 0) && (
                <div className="grid-2 mt-5">
                  {upanamaGenData.length > 0 && (
                    <Card title="Upanama (General)" subtitle="General upanama distribution" onReport={() => goCustom?.(["religious-details"], "user")}>
                  ...
                  {upanaPropData.length > 0 && (
                    <Card title="Upanama (Proper)" subtitle="Proper clan name distribution" onReport={() => goCustom?.(["religious-details"], "user")}>
              ...
              {demiGodData.length > 0 && (
                <Card title="Demi God (Daiva) Distribution" subtitle="Family ancestral demi gods worshipped" style={{ marginTop: 18 }} onReport={() => goCustom?.(["religious-details"], "user")}>
              ...
              {religious.ancestralStats && ((religious.ancestralStats.withChallenge || 0) + (religious.ancestralStats.withoutChallenge || 0) > 0) && (
                <Card title="Ancestral Challenge Breakdown" subtitle="Families with / without ancestral challenges" style={{ marginTop: 18 }} onReport={() => goCustom?.(["religious-details"], "user")}>
                  <div className="grid-2">
...
// ─── SanghaTab ────────────────────────────────────────────────────────────────
function SanghaTab({ data, loading, onGoToCustomReport }: {
  data: AdminSanghaReport | null;
  loading: boolean;
  onGoToCustomReport?: (sections: string[], category?: string) => void;
}) {
    ...
    <div className="adv-flex">
      ...
      <div className="adv-content">
        ...
        <section className="adv-section" id="as-status">
          ...
          <Card title="Sangha Status Distribution" subtitle="Approval pipeline" onReport={() => goCustom?.(["sangha-details"], "sangha")}>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
        ...
        <section className="adv-section" id="as-members">
          <div className="adv-sec-header">
            <div className="adv-sec-icon"><Users style={{ width: 18, height: 18, color: "#0ea5e9" }} /></div>
            <div><div className="adv-sec-title">Sangha Members by Role</div><div className="adv-sec-sub">Top sanghas by member count — filter to compare any sangha</div></div>
          </div>
        ...
        <section className="adv-section" id="as-member-type">
          <div className="adv-sec-header">
            ...
            <div>
              <div className="adv-sec-title">Full-Time & Part-Time Members</div>
              <div className="adv-sec-sub">Per sangha breakdown — Male · Female — Full-Time (blue) · Part-Time (amber)</div>
            </div>
          </div>
...
export default function AdminAdvancedDashboard({
  initialSection, onSectionRendered, onGoToCustomReport,
  ...
  useScrollToSection(initialSection, activeTab, setActiveTab, onSectionRendered);
  ...
  const loadSanghas = useCallback(async () => {
    ...
    } finally { setSanghaLoading(false); }
  }, []);
    ...
    <>
      ...
      <div className="adv-root">
        <div className="adv-topbar">
          ...
          <button className="adv-refresh-btn" onClick={() => { activeTab === "users" ? loadUsers() : loadSanghas(); }}>
        ...
        <div className="adv-body">
          {activeTab === "users" && (
            <UserTab data={userReport} loading={userLoading} onGoToCustomReport={onGoToCustomReport} />
          )}
          {activeTab === "sanghas" && (
            <SanghaTab data={sanghaReport} loading={sanghaLoading} onGoToCustomReport={onGoToCustomReport} />
          )}
        </div>
      </div>
    </>

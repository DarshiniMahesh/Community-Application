...
//   GET /admin/reports/advanced  → getAdminAdvancedReportsuser
//   GET /admin/reports/sanghas   → getAdminSanghaReports
// ─────────────────────────────────────────────────────────────────────────────
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 900px) {
    .grid-2, .grid-4, .grid-2-1 { grid-template-columns: 1fr; }
  }
  ...
  .geo-chip {
    ...
    border: 1px solid var(--teal-lt); background: var(--teal-lt);
...
// ─── API layer ────────────────────────────────────────────────────────────────
// Routes: GET /admin/reports/advanced  and  GET /admin/reports/sanghas
// (see adminreport.js — uses authenticate + requireRole('admin'))

async function fetchAdminUserReport(token: string): Promise<AdminUserReport> {
  const res = await fetch('/api/admin/reports/advanced', {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error(`Failed to load user report (${res.status})`);
  return res.json();
}

async function fetchAdminSanghaReport(token: string): Promise<AdminSanghaReport> {
  const res = await fetch('/api/admin/reports/sanghas', {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error(`Failed to load sangha report (${res.status})`);
  return res.json();
}
...
function SectionHeader({ id, icon: Icon, title, subtitle, color }: { id: string; icon: any; title: string; subtitle: string; color: string }) {
  return (
    <div id={id} className="adv-sec-header adv-section">
      <div className="adv-sec-icon"><Icon style={{ width: 18, height: 18, color }} /></div>
      <div>
        <div className="adv-sec-title">{title}</div>
        <div className="adv-sec-sub">{subtitle}</div>
      </div>
    </div>
  );
}
...
// Gender stacked vertical bar (long labels)
...
function UserTab({ data, loading }: { data: AdminUserReport | null; loading: boolean }) {
  ...
  const maritalBarData = maritalGenderData.length > 0 ? maritalGenderData
  ...
  const professionBarData = professionGenderData.length > 0 ? professionGenderData
  ...
  const employmentBarData = employmentGenderData.length > 0 ? employmentGenderData
    ...
    <div className="adv-flex">
      ...
      <div className="adv-content">
        ...
        <section className="adv-section" id="au-geographic">
          ...
          <Card title={selectedCities.length === 0 ? "Top Cities by Family Count" : `${selectedCities.length} Cities Selected`}
            subtitle="Male · Female — geographic spread">
        ...
        <section className="adv-section" id="as-status">
          ...
          <div className="grid-2">
            <Card title="Sangha Status Distribution" subtitle="Approval pipeline">
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={data.statusBreakdown.map(s => ({
                    name: SANGHA_STATUS_LABELS[s.status] ?? s.status,
                    value: s.count,
                    color: SANGHA_STATUS_COLORS[s.status] ?? "#94a3b8",
        ...
        <section className="adv-section" id="as-geographic">
          ...
          <div className="grid-2">
            <Card title="Sanghas by State" subtitle="Top states" badge={`${data.stateDistribution?.length || 0} states`}>
              <SimpleBar
                data={(data.stateDistribution || []).slice(0, 12).map(r => ({ label: r.state || "Unknown", count: r.count }))}
                palette={STATE_PALETTE}
                height={Math.max(220, Math.min((data.stateDistribution?.length || 0), 12) * 34)}
              />
            </Card>
            <Card title="Sanghas by District" subtitle="Top districts" badge={`${data.districtDistribution?.length || 0} districts`}>
              <SimpleBar
                data={(data.districtDistribution || []).slice(0, 12).map(r => ({ label: r.district || "Unknown", count: r.count }))}
                palette={GEO_PALETTE}
                height={Math.max(220, Math.min((data.districtDistribution?.length || 0), 12) * 34)}
              />
            </Card>
          </div>
        </section>
        ...
        <section className="adv-section" id="as-members">
          ...
          <Card title="Top Sanghas by Member Count" subtitle="Total registered & approved profiles">
            <ResponsiveContainer width="100%" height={Math.max(260, Math.min((data.sanghasByMemberCount || []).length, 15) * 36)}>
              <BarChart
                data={(data.sanghasByMemberCount || []).slice(0, 15).map(s => ({
                  label: s.sangha_name.length > 20 ? s.sangha_name.slice(0, 18) + "…" : s.sangha_name,
                  "Total": s.member_count,
                  "Approved": s.approved_count,
                }))}
                layout="vertical"
                margin={{ left: 8, right: 30, top: 5, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10, fill: "#94a3b8" }} tickLine={false} axisLine={false} allowDecimals={false} />
            ...
            {(data.completionRates || []).length > 0 && (
              <Card title="Average Profile Completion" subtitle="By sangha — top 10">
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart
                    data={(data.completionRates || []).slice(0, 10).map(s => ({
                      label: s.sangha_name.length > 18 ? s.sangha_name.slice(0, 16) + "…" : s.sangha_name,
                    ...
                    <Bar dataKey="Completion %" radius={[0, 6, 6, 0]} maxBarSize={20}>
                      {(data.completionRates || []).slice(0, 10).map((_: any, i: number) => (
                        <Cell key={i} fill={`hsl(${260 + i * 8}, 70%, ${55 + i * 2}%)`} />
                      ))}
                      <LabelList dataKey="Completion %" position="right"
                      ...
                      <tr key={i}>
                        ...
                        <td className="center">
                          <span className="status-chip"
                            style={{
                              background: (SANGHA_STATUS_COLORS[s.status] ?? "#94a3b8") + "20",
                              color: SANGHA_STATUS_COLORS[s.status] ?? "#94a3b8",
                            }}>
...
export default function AdminAdvancedDashboard({ token }: AdminDashboardProps) {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try   { setUserReport(await fetchAdminUserReport(token)); }
  ...
  const loadSanghas = useCallback(async () => {
    ...
    try   { setSanghaReport(await fetchAdminSanghaReport(token)); }
    catch (e: any) { setSanghaError(e.message); }
  ...
  useEffect(() => {
    if (activeTab === "sanghas" && !sanghaReport && !sanghaLoading) loadSanghas();
  }, [activeTab, sanghaReport, sanghaLoading, loadSanghas]);

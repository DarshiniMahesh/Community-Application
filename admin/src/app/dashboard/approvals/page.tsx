...
export default function ApprovalsPage() {
  ...
  return (
    <div className="page">
      <div className="page-header"><h1>Approve Sangha&apos;s and Users</h1></div>
            ...
            <div className="approval-card" key={u.id}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <button className="btn btn-secondary btn-sm" onClick={() => setViewModal(u)}>See</button>
                </div>
              </div>
            </div>
          ))
      ...
      {viewModal && (
        <Modal open title={`User Info — ${viewModal.id}`} onClose={() => setViewModal(null)}
          footer={<button className="btn btn-secondary" onClick={() => setViewModal(null)}>Close</button>}>
          {([['Name', viewModal.name], ['Email', viewModal.email], ['Phone', viewModal.phone], ['Submitted', viewModal.submitted]] as [string, string][]).map(([k, v]) => (

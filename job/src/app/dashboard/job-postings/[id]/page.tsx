...
import { ArrowLeft, Edit2, Eye, Users } from "lucide-react";
...
export default function JobDetailPage() {
  ...
  const handleStatusUpdate = async (applicantId: string, newStatus: string, currentStatus: string) => {
    ...
    try {
      await api.patch(`/company/jobs/${jobId}/applicants/${applicantId}/status`, { status: newStatus });
      setApplicants((prev) =>
    ...
    <div>
      ...
      <div style={styles.pageHeader}>
        ...
        <div style={styles.headerRight}>
          <Link href={`/dashboard/job-postings/${jobId}?edit=true`} style={styles.editBtn}>
      ...
      <div style={styles.tabs}>
        {(["details", "applicants"] as const).map((tab) => (
          ...
          >
            ...
            {tab === "details" ? "Job Details" : `Applicants (${applicants.length})`}
          </button>
        ))}
      </div>
...
// Pulled out of styles object to avoid the function-in-Record type error
